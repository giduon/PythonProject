...
print('기존 파일에 커피별 특성 추가합니다.')  # append text
coffee_file2 = open(file=filename, mode='at', encoding='UTF-8')

for idx in range(len(coffees)):
    if idx % 2 == 0:
        message = f'{idx+1}번째 커피 "{coffees[idx]}" : 풍미가 깊어요.\n'
    else:
        message = f'{idx+1}번째 커피 "{coffees[idx]}" : 부드럽게 즐기세요.\n'
    coffee_file2.write(message)
# end for
